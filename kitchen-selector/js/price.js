/**
 * price.js — 厨房价格矩阵解析工具（纯函数，无 DOM 依赖）
 *
 * 处理三类价格表示：
 *  1. 数字 priceDiff：相对基本仕様的差价（正=加价 负=减价 0=基本）
 *  2. 文本价格单元格："＋￥10,000" / "－￥5,000" / "基本仕様" / "-" / "unknown" / "＋￥0"
 *  3. 矩阵 priceMatrix：键如 "1800" / "1800 R/L"（按 t2 数据约定，兼容尺寸×位置）
 *
 * 用法（作为全局命名空间 KITCHEN.price 挂载）：
 *   在 <script src="js/price.js"> 之后 window.KITCHEN = window.KITCHEN || {}; KITCHEN.price = {...}
 */
(function () {
  'use strict';
