/**
 * wizard.js — LIXIL Renobio Fit（リノビオフィット）整体浴室选型报价 UI（elf_D老肖的世界 风格，分步向导 + 报价单 两个标签页）
 *
 * 视图：选型向导（横向步骤条 + 选项区 + 右侧合计栏）/ 报价单（見積書）
 * 依赖：window.RENOBIO_DATA、window.RENOBIO.price、window.RENOBIO.quote
 * 暴露：window.RENOBIO.wizard = { init, renderAll, renderQuote, state }
 *
...
(function () {
  ...
  /* ---------------- 初始化（事件只绑一次） ---------------- */
  function init(data) {
    Q = window.RENOBIO.quote;
    P = window.RENOBIO.price;
    Q.init(data);
    els = {
  ...
  function step0Info() {
    ...
    html += '</div>';
    html += '<p class="base-meta">※ ' + t('1216 Nタイプ 標準仕様価格 ¥841,500（税抜）；N＞B＞T＞C（同サイズ）；寒冷地 ＋¥5,000', '1216 Nタイプ 標準仕様価格 ¥841,500（税抜）；N＞B＞T＞C（同サイズ）；寒冷地 ＋¥5,000') + '</p>';
    html += '</div>';
  ...
  function wallPatternHtml(d) {
    ...
    var basicList = patterns.filter(function (p) { return p.class !== 'high'; });
  ...
  function renderQuote() {
    ...
    html += '<div class="doc-header">' +
      ...
      '<h1>' + t('報 价 单', '見 積 書') + '</h1>' +
      '<div class="doc-sub">LIXIL Renobio Fit（リノビオフィット）／ 整体浴室（公寓改造用）</div>' +
      '<div class="doc-meta">' +
  ...
  function copyList() {
    var r = Q.computeQuote();
    var lines = ['【LIXIL Renobio Fit（リノビオフィット）見積（报价）选型清单】'];
