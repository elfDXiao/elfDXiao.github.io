/**
 * wizard.js — TOTO シャワールーム（Shower Room）选型报价 UI（elf_D老肖的世界 风格，分步向导 + 报价单 两个标签页）
 *
 * 视图：选型向导（横向步骤条 + 选项区 + 右侧合计栏）/ 报价单（見積書）
 * 依赖：window.SHOWER_DATA、window.SHOWER.price、window.SHOWER.quote
 * 暴露：window.SHOWER.wizard = { init, renderAll, renderQuote, state }
 *
...
(function () {
  ...
  /* ---------------- 初始化（事件只绑一次） ---------------- */
  function init(data) {
    Q = window.SHOWER.quote;
    P = window.SHOWER.price;
    Q.init(data);
    els = {
  ...
  function renderQuote() {
    ...
    html += '<div class="doc-header">' +
      ...
      '<h1>' + t('報 价 单', '見 積 書') + '</h1>' +
      '<div class="doc-sub">TOTO シャワールーム（Shower Room）／ 淋浴房</div>' +
      '<div class="doc-meta">' +
  ...
  function copyList() {
    var r = Q.computeQuote();
    var lines = ['【TOTO シャワールーム 見積（报价）选型清单】'];
