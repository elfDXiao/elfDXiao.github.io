/**
 * wizard.js — LIXIL Lidea（リデア）整体浴室选型报价 UI（elf_D老肖的世界 风格，分步向导 + 报价单 两个标签页）
 *
 * 视图：选型向导（横向步骤条 + 选项区 + 右侧合计栏）/ 报价单（見積書）
 * 依赖：window.LIDEA_DATA、window.LIDEA.price、window.LIDEA.quote
 * 暴露：window.LIDEA.wizard = { init, renderAll, renderQuote, state }
 *
...
(function () {
  ...
  /* ---------------- 初始化（事件只绑一次） ---------------- */
  function init(data) {
    Q = window.LIDEA.quote;
    P = window.LIDEA.price;
    Q.init(data);
    els = {
  ...
  function step0Info() {
    var html = '<div class="opt-block" style="margin-bottom:0;">' +
      '<div class="dim-title">' + t('基本セット価格（税抜・タイプ×サイズ・取付・設置費別）', '基本セット価格（税抜・タイプ×サイズ・取付・設置費別）') + '</div>' +
      '<div class="size-grid">';
    ...
    html += '</div>';
    html += '<p class="base-meta">※ ' + t('1616 Mタイプ 標準仕様価格 ¥1,379,000（税抜）；1624/S1818 仅 M/C タイプ', '1616 Mタイプ 標準仕様価格 ¥1,379,000（税抜）；1624/S1818 は M/C タイプのみ') + '</p>';
    html += '</div>';
  ...
  function renderQuote() {
    ...
    html += '<div class="doc-header">' +
      ...
      '<h1>' + t('報 价 单', '見 積 書') + '</h1>' +
      '<div class="doc-sub">LIXIL Lidea（リデア）／ 整体浴室</div>' +
      '<div class="doc-meta">' +
  ...
  function copyList() {
    var r = Q.computeQuote();
    var lines = ['【LIXIL Lidea（リデア）見積（报价）选型清单】'];
