...
r = Q.computeQuote();
assert('T1620 + EGAA1(+105,000，P/T/S/F 同价) → 1,554,000', r.totalEx === 1554000, r.totalEx);
Q.reset();
...
console.log('== 壁柄花纹级（sazanaWallPatterns） ==');
assert('accentPatterns 37 柄 + surroundPatterns 8 柄', DATA.sazanaWallPatterns.accentPatterns.length === 37 && DATA.sazanaWallPatterns.surroundPatterns.length === 8);
assert('accentPriceMatrix 4×3', Object.keys(DATA.sazanaWallPatterns.accentPriceMatrix).length === 4);
Q.state.sel.type = 'P';
Q.state.sel.wall = 'EGAA1';
r = Q.computeQuote();
assert('4面同色 EGAA1 P → +105,000 → 1,741,000', r.totalEx === 1741000, r.totalEx);
Q.reset();
Q.state.sel.type = 'P';
...
assert('P タイプ FRONT_ACCENT ACC_PRE × 周辺HⅡ = +73,500（L1=P/T/S/F）', Q.contributionFor('wall') === 73500, Q.contributionFor('wall'));
Q.state.sel.type = 'T';
assert('T タイプ FRONT_ACCENT ACC_PRE × 周辺HⅡ = +73,500（T も L1）', Q.contributionFor('wall') === 73500, Q.contributionFor('wall'));
Q.state.sel.type = 'N';
assert('N タイプ FRONT_ACCENT ACC_PRE × 周辺HⅡ = +94,500（L2=N）', Q.contributionFor('wall') === 94500, Q.contributionFor('wall'));
Q.reset();
Q.state.sel.type = 'P';
Q.state.sub.wall_plan = 'FRONT_ACCENT';
Q.state.sel.wall = 'ACC_BASIC';
Q.state.sub.wall_pattern = 'EG2J3';
Q.state.sub.wall_surround = '周辺ベーシックグレード';
assert('P タイプ ACC_BASIC × 周辺BASIC = −21,000（L1=P/T/S/F）', Q.contributionFor('wall') === -21000, Q.contributionFor('wall'));
Q.state.sub.wall_pattern = 'EG2J4';
assert('柄 EG2J4 でも同額 −21,000（柄による加算なし）', Q.contributionFor('wall') === -21000, Q.contributionFor('wall'));
Q.state.sel.type = 'N';
assert('N タイプ EG2J4 × 周辺BASIC = 0（L2=N）', Q.contributionFor('wall') === 0, Q.contributionFor('wall'));
Q.reset();
Q.state.sub.wall_plan = 'FRONT_ACCENT';
Q.state.sel.wall = 'ACC_PRE';
assert('ACC_* 未选周辺 → null（待选择）', Q.contributionFor('wall') == null);
Q.reset();
assert('wall 分类 49 选项（3 模式 + 39 柄 4面同色 + 4 ACC + 3 SHUHEN）', Q.cat('wall').options.length === 49, Q.cat('wall').options.length);
assert('4面同色柄 EGAB5 priceByType P/T/S/F=63,000・N=84,000', ['P','T','S','F'].every(t => P.priceFor(Q.cat('wall').options.find(o => o.code === 'EGAB5'), t, '1620') === 63000) && P.priceFor(Q.cat('wall').options.find(o => o.code === 'EGAB5'), 'N', '1620') === 84000);
Q.state.sel.type = 'P';
Q.state.sel.wall = 'EGAB5';
r = Q.computeQuote();
assert('P タイプ EGAB5 → +63,000 → 1,699,000', r.totalEx === 1699000, r.totalEx);
assert('4面同色柄明细含品番 EGAB5', Q.describe('wall').model === 'EGAB5', Q.describe('wall').model);
Q.reset();
Q.state.sub.wall_plan = 'FRONT_ACCENT';
Q.state.sel.wall = 'ACC_PRE';
Q.state.sub.wall_pattern = 'EG2J1';
Q.state.sub.wall_surround = '周辺ベーシックグレード';
var desc5 = Q.describe('wall');
assert('アクセント柄明细含品番 EG2J1', desc5.model === 'EG2J1', desc5.model);
assert('アクセント柄明细含花纹名 マチエルレッド', (desc5.nameJa || '').indexOf('マチエルレッド') >= 0, desc5.nameJa);
Q.reset();
