...
assert('P タイプ ACC_PRE × 周辺HⅡ = +73,500（L1）', Q.contributionFor('wall') === 73500, Q.contributionFor('wall'));
Q.state.sel.type = 'T';
assert('T タイプ ACC_PRE × 周辺HⅡ = +94,500（L2）', Q.contributionFor('wall') === 94500, Q.contributionFor('wall'));
Q.reset();
Q.state.sel.type = 'P';
Q.state.sel.wall = 'ACC_BASIC';
Q.state.sub.wall_surround = '周辺ベーシックグレード';
assert('P タイプ ACC_BASIC × 周辺BASIC = −21,000', Q.contributionFor('wall') === -21000, Q.contributionFor('wall'));
Q.reset();
Q.state.sel.wall = 'ACC_PRE';
assert('ACC_* 未选周辺 → null（待选择）', Q.contributionFor('wall') == null);
Q.reset();
assert('wall 分类 46 选项（39 柄 4面同色 + 4 ACC + 3 SHUHEN）', Q.cat('wall').options.length === 46, Q.cat('wall').options.length);
