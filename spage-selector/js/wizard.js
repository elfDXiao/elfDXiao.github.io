/**
 * wizard.js — LIXIL SPAGE（スパージュ）整体浴室选型报价 UI（elf_D老肖的世界 风格，分步向导 + 报价单 两个标签页）
 *
 * 视图：选型向导（横向步骤条 + 选项区 + 右侧合计栏）/ 报价单（見積書）
 * 依赖：window.SPAGE_DATA、window.SPAGE.price、window.SPAGE.quote
 * 暴露：window.SPAGE.wizard = { init, renderAll, renderQuote, state }
 *
...
(function () {
  ...
  /* ---------------- 初始化（事件只绑一次） ---------------- */
  function init(data) {
    Q = window.SPAGE.quote;
    P = window.SPAGE.price;
    Q.init(data);
    els = {
  ...
  /* 步骤 0：尺寸卡片 + タイプ选项（套装价 = タイプ×サイズ×設置矩阵） */
  function step0Info() {
    var html = '<div class="opt-block" style="margin-bottom:0;">' +
      '<div class="dim-title">' + t('標準仕様価格（税抜・タイプ×サイズ×設置・取付・設置費別）', '標準仕様価格（税抜・タイプ×サイズ×設置・取付・設置費別）') + '</div>' +
      '<div class="size-grid">';
    ...
    // 設置・地域区分（step 0 的其他维度）
    var idm = Q.dim('install');
    if (idm) html += dimensionHtml(idm);
    var rd = Q.dim('region');
  ...
  function renderSummary() {
    ...
    if (els.sumType) els.sumType.textContent = r.type + 'タイプ' + (r.install === 'M' ? '（マンション）' : '（戸建）');
  ...
  function renderQuote() {
    ...
    html += '<div class="doc-header">' +
      ...
      '<h1>' + t('報 价 单', '見 積 書') + '</h1>' +
      '<div class="doc-sub">LIXIL SPAGE（スパージュ）／ 整体浴室</div>' +
      '<div class="doc-meta">' +
  ...
  function copyList() {
    var r = Q.computeQuote();
    var lines = ['【LIXIL SPAGE（スパージュ）見積（报价）选型清单】'];
