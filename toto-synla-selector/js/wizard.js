/**
 * wizard.js — TOTO Synla（シンラ）整体浴室选型报价 UI（elf_D老肖的世界 风格，分步向导 + 报价单 两个标签页）
 *
 * 视图：选型向导（横向步骤条 + 选项区 + 右侧合计栏）/ 报价单（見積書）
 * 依赖：window.SYNLA_DATA、window.SYNLA.price、window.SYNLA.quote
 * 暴露：window.SYNLA.wizard = { init, renderAll, renderQuote, state }
 *
 * 事件绑定架构（避免监听器累积卡死）：
...
(function () {
  ...
  /* ---------------- 初始化（事件只绑一次） ---------------- */
  function init(data) {
    Q = window.SYNLA.quote;
    P = window.SYNLA.price;
    Q.init(data);
    els = {
  ...
  function bindDelegated() {
    ...
    body.addEventListener('change', function (e) {
      ...
      // wall：plan 单选
      if (tgt.name === 'wall_plan' && tgt.hasAttribute('data-code')) {
        Q.state.sel.wall = tgt.getAttribute('data-code');
        delete Q.state.sub.wall;      // 切 plan 清柄/周辺
        renderStep(); renderSummary();
        return;
      }
      // wall：柄选择
      if (tgt.hasAttribute('data-wall-mark')) {
        var ws = Q.state.sub.wall || {};
        ws.mark = tgt.getAttribute('data-wall-mark');
        Q.state.sub.wall = ws;
        renderStep(); renderSummary();
        return;
      }
      // wall：周辺グレード
      if (tgt.hasAttribute('data-wall-grade')) {
        var ws2 = Q.state.sub.wall || {};
        ws2.grade = tgt.getAttribute('data-wall-grade');
        Q.state.sub.wall = ws2;
        renderStep(); renderSummary();
        return;
      }
    });
  ...
  function step0Info() {
    var html = '<div class="opt-block" style="margin-bottom:0;">' +
      '<div class="dim-title">' + t('基本セット価格（税抜・タイプ×サイズ・取付・設置費別・不含窗）', '基本セット価格（税抜・タイプ×サイズ・取付・設置費別）') + '</div>' +
      '<div class="size-grid">';
    ...
    html += '</div>';
    html += '<p class="base-meta">※ ' + t('1616 Gタイプ 基本套装价 ¥3,128,000（税抜）；1317/1216 仅 D/C タイプ', '1616 Gタイプ 基本セット価格 ¥3,128,000（税抜）；1317/1216 は D/C タイプのみ') + '</p>';
    html += '</div>';

    var d = Q.dim('type');
    html += '<div class="opt-block" data-dim="type">' +
      '<div class="dim-title">' + t(d.titleZh, d.titleJa) + '（' + tp('决定套装价', '基本セット価格を決定') + '）</div>';
  ...
  function dimensionHtml(d) {
    ...
    switch (d.kind) {
      ...
      case 'wall': html += wallHtml(d); break;
      default: break;
    }
    html += '</div>';
    return html;
  ...
  /* 壁柄：plan 单选 + 柄 chips + 周辺グレード chips */
  function wallHtml(d) {
    var html = '<div class="opt-grid">';
    var codes = Q.codesOf(d);
    codes.forEach(function (code) {
      var plan = Q.opt(d.id, code);
      if (!plan) return;
      var sel = Q.state.sel.wall === code ? ' on' : '';
      var dis = Q.disabledReason(d.id, code);
      var disCls = dis ? ' disabled' : '';
      html += '<label class="opt-card' + sel + disCls + '">' +
        '<input type="radio" name="wall_plan" data-code="' + esc(code) + '"' + (sel ? ' checked' : '') + (dis ? ' disabled' : '') + '>' +
        '<span class="opt-name">' + esc(plan.name_zh || plan.code) + '<em>' + esc(plan.name_ja || '') + '</em></span>' +
        '<span class="opt-code">' + esc(plan.subOptions.length + ' 柄') + '</span>' +
        '</label>';
    });
    html += '</div>';

    // 选中 plan 后：柄选择
    var planCode = Q.state.sel.wall;
    if (planCode) {
        ...
        Object.keys(byGrade).forEach(function (g) {
          ...
          byGrade[g].forEach(function (s) {
            ...
            var price = planCode === '4SAME'
              ? (function () { var v = P.priceByTypeValue(s, Q.typeCode()); return typeof v === 'number' ? P.fmtDiff(v) : '—'; })()
              : tp('选择周辺后计价', '周辺選択で価格確定');
  ...
  function renderQuote() {
    ...
    html += '<div class="doc-header">' +
      ...
      '<h1>' + t('報 价 单', '見 積 書') + '</h1>' +
      '<div class="doc-sub">TOTO Synla（シンラ）システムバス ／ 整体浴室</div>' +
      '<div class="doc-meta">' +
      ...
      '<span>' + t('架台 / 架台：', '架台：') + esc(r.pedestal) + '</span>' +
  ...
  function copyList() {
    var r = Q.computeQuote();
    var lines = ['【TOTO Synla システムバス 見積（报价）选型清单】'];
    lines.push('品番：' + Q.productNo());
    lines.push('尺寸：' + r.size + ' ｜ 型号：' + r.type + 'タイプ ｜ 门位置：' + r.doorPos + ' ｜ 架台：' + r.pedestal);
